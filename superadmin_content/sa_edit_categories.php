...
if(isset($_GET['delete'])){
    ...
    $check_posts = $conn->prepare("SELECT COUNT(*) as post_count FROM `posts` WHERE category_id = ?");
    $check_posts->execute([$category_id]);
    $post_count = $check_posts->fetch(PDO::FETCH_ASSOC)['post_count'];
    
    // Check if category is used in articles
    $check_articles = $conn->prepare("SELECT COUNT(*) as article_count FROM `articles` WHERE category_id = ?");
    $check_articles->execute([$category_id]);
    $article_count = $check_articles->fetch(PDO::FETCH_ASSOC)['article_count'];
    
    // Check if category is used in tejido
    $check_tejido = $conn->prepare("SELECT COUNT(*) as tejido_count FROM `tejido` WHERE category_id = ?");
    $check_tejido->execute([$category_id]);
    $tejido_count = $check_tejido->fetch(PDO::FETCH_ASSOC)['tejido_count'];
    
    // Check if category is used in e_magazines
    $check_magazines = $conn->prepare("SELECT COUNT(*) as magazine_count FROM `e_magazines` WHERE category_id = ?");
    $check_magazines->execute([$category_id]);
    $magazine_count = $check_magazines->fetch(PDO::FETCH_ASSOC)['magazine_count'];
    
    $total_count = $post_count + $article_count + $tejido_count + $magazine_count;
    
    if($total_count > 0){
        $message[] = "Cannot delete category - it contains $total_count item(s)!";
    } else {
        // Delete category
...
<section class="add-tejido">
    ...
    <form action="" method="post">
        ...
        <?php if($edit_mode): ?>
            <a href="sa_edit_categories.php" class="option-btn" style="margin-top: 1rem;">Cancel Edit</a>
        <?php endif; ?>
    </form>
</section>
    ...
    <div class="box-container">
        ...
        if($select_categories->rowCount() > 0){
            while($category = $select_categories->fetch(PDO::FETCH_ASSOC)){
                ...
                $count_articles = $conn->prepare("SELECT COUNT(*) as article_count FROM `articles` WHERE category_id = ?");
                $count_articles->execute([$category['category_id']]);
                $article_count = $count_articles->fetch(PDO::FETCH_ASSOC)['article_count'];
        ...
        <div class="box">
            ...
            <p>Articles: <span><?= $article_count; ?></span></p>
            <p>Tejidos: <span><?= $tejido_count; ?></span></p>
            <p>E-Magazines: <span><?= $magazine_count; ?></span></p>
            
            <div class="flex-btn">
                <a href="sa_edit_categories.php?edit=<?= $category['category_id']; ?>" class="btn">Edit</a>
                <a href="sa_edit_categories.php?delete=<?= $category['category_id']; ?>" 
                   class="delete-btn" 
                   <?= $total_count > 0 ? 'disabled style="opacity:0.5;cursor:not-allowed;" onclick="alert(\'Cannot delete a category that contains items.\'); return false;"' : 
