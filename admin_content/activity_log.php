<?php
// Improve session security
if (session_status() == PHP_SESSION_NONE) {
    session_start();
}

require_once '../classes/database.class.php';

// Modified role check to allow only subadmins
if (empty($_SESSION['admin_id']) || $_SESSION['admin_role'] !== 'subadmin') {
    header('Location: ../access_denied.php');
    exit();
}

// Get current admin ID
$current_admin_id = $_SESSION['admin_id'];

// DB connection
$db = new Database();
$conn = $db->connect();

// Sanitize and validate inputs
$records_per_page = 10;
$page = filter_input(INPUT_GET, 'page', FILTER_VALIDATE_INT, [
    'options' => [
        'min_range' => 1,
        'default' => 1
    ]
]);

$offset = ($page - 1) * $records_per_page;

// Simplified filters for subadmins
$type_filter = filter_input(INPUT_GET, 'type', FILTER_SANITIZE_STRING) ?? 'all';
$date_filter = filter_input(INPUT_GET, 'date', FILTER_SANITIZE_STRING) ?? 'all';

// Date condition logic
$date_condition = '';
switch ($date_filter) {
    case 'today':
        $date_condition = "AND DATE(timestamp) = CURDATE()";
        break;
    case 'this_week':
        $date_condition = "AND timestamp >= DATE_SUB(CURDATE(), INTERVAL WEEKDAY(CURDATE()) DAY)";
        break;
    case 'this_month':
        $date_condition = "AND MONTH(timestamp) = MONTH(CURDATE()) AND YEAR(timestamp) = YEAR(CURDATE())";
        break;
    case 'past_3_months':
        $date_condition = "AND timestamp >= DATE_SUB(CURDATE(), INTERVAL 3 MONTH)";
        break;
}

try {
    // Modified query to only show activities related to the current subadmin
    $type_condition = ($type_filter !== 'all') ? "AND type = :type_filter" : "";

    $logs_query = $conn->prepare("
        SELECT * FROM (
            (SELECT 
                'Post' AS type,
                p.title,
                p.created_at AS timestamp,
                p.status,
                a.firstname,
                a.lastname,
                a.user_name,
                'Created' AS action
            FROM posts p
            JOIN accounts a ON p.created_by = a.account_id
            WHERE p.created_by = :admin_id)

            UNION ALL

            (SELECT 
                'Post' AS type,
                p.title,
                p.updated_at AS timestamp,
                p.status,
                a.firstname,
                a.lastname,
                a.user_name,
                'Updated' AS action
            FROM posts p
            JOIN accounts a ON p.created_by = a.account_id
            WHERE p.created_at != p.updated_at AND p.created_by = :admin_id2)

            UNION ALL

            (SELECT 
                'Article' AS type,
                ar.title,
                ar.created_at AS timestamp,
                ar.status,
                a.firstname,
                a.lastname,
                a.user_name,
                'Created' AS action
            FROM articles ar
            JOIN accounts a ON ar.created_by = a.account_id
            WHERE ar.created_by = :admin_id3)

            UNION ALL

            (SELECT 
                'Comment' AS type,
                p.title,
                c.commented_at AS timestamp,
                'commented' AS status,
                a.firstname,
                a.lastname,
                a.user_name,
                'Commented' AS action
            FROM comments c
            JOIN posts p ON c.post_id = p.post_id
            JOIN accounts a ON c.commented_by = a.account_id
            WHERE c.commented_by = :admin_id4)

            UNION ALL

            (SELECT 
                'Like' AS type,
                p.title,
                l.created_at AS timestamp,
                'liked' AS status,
                a.firstname,
                a.lastname,
                a.user_name,
                'Liked' AS action
            FROM likes l
            JOIN posts p ON l.post_id = p.post_id
            JOIN accounts a ON l.account_id = a.account_id
            WHERE l.account_id = :admin_id5)

        ) AS all_logs
        WHERE 1=1 $date_condition $type_condition
        ORDER BY timestamp DESC
        LIMIT :limit OFFSET :offset
    ");

    // Bind parameters with explicit type casting
    $logs_query->bindValue(':admin_id', $current_admin_id, PDO::PARAM_INT);
    $logs_query->bindValue(':admin_id2', $current_admin_id, PDO::PARAM_INT);
    $logs_query->bindValue(':admin_id3', $current_admin_id, PDO::PARAM_INT);
    $logs_query->bindValue(':admin_id4', $current_admin_id, PDO::PARAM_INT);
    $logs_query->bindValue(':admin_id5', $current_admin_id, PDO::PARAM_INT);
    $logs_query->bindValue(':limit', $records_per_page, PDO::PARAM_INT);
    $logs_query->bindValue(':offset', $offset, PDO::PARAM_INT);
    
    if ($type_filter !== 'all') {
        $logs_query->bindValue(':type_filter', $type_filter, PDO::PARAM_STR);
    }

    $logs_query->execute();
    $activity_logs = $logs_query->fetchAll(PDO::FETCH_ASSOC);

    // Count total logs for pagination (only for this subadmin)
    $count_query = $conn->prepare("
        SELECT (
            (SELECT COUNT(*) FROM posts WHERE created_by = :admin_id) +
            (SELECT COUNT(*) FROM articles WHERE created_by = :admin_id2) +
            (SELECT COUNT(*) FROM comments WHERE commented_by = :admin_id3) +
            (SELECT COUNT(*) FROM likes WHERE account_id = :admin_id4)
        ) AS total_logs
    ");
    
    $count_query->bindValue(':admin_id', $current_admin_id, PDO::PARAM_INT);
    $count_query->bindValue(':admin_id2', $current_admin_id, PDO::PARAM_INT);
    $count_query->bindValue(':admin_id3', $current_admin_id, PDO::PARAM_INT);
    $count_query->bindValue(':admin_id4', $current_admin_id, PDO::PARAM_INT);
    $count_query->execute();
    $total_logs = $count_query->fetch(PDO::FETCH_ASSOC)['total_logs'];

    // Add a reasonable page limit
    $total_pages = ceil($total_logs / $records_per_page);

} catch (PDOException $e) {
    // Improved error handling
    error_log("Activity Logs Error: " . $e->getMessage());
    
    // User-friendly error message
    $_SESSION['error_message'] = "An error occurred while retrieving activity logs. Please try again later.";
    header('Location: ../error.php');
    exit();
}
?>
<!DOCTYPE html>
<html lang="en">
<head>
    <meta charset="UTF-8">
    <title>My Activity Log | Subadmin</title>
    <link rel="stylesheet" href="../css/admin_style.css">
    <link rel="stylesheet" href="https://cdnjs.cloudflare.com/ajax/libs/font-awesome/6.1.1/css/all.min.css">
    <style>
        .activity-logs {
            padding: 20px;
        }
        .activity-logs-table {
            width: 100%;
            border-collapse: collapse;
            margin-top: 20px;
        }
        .activity-logs-table th, .activity-logs-table td {
            padding: 12px;
            text-align: left;
            border-bottom: 1px solid #ddd;
        }
        .activity-logs-table th {
            background-color: var(--light-bg);
        }
        .search-container {
            margin-bottom: 20px;
            display: flex;
            gap: 10px;
        }
        .search-container select, .search-container input {
            padding: 8px;
            border-radius: 4px;
            border: 1px solid #ddd;
        }
        .pagination {
            margin-top: 20px;
            display: flex;
            gap: 5px;
            justify-content: center;
        }
        .pagination a {
            padding: 8px 12px;
            border: 1px solid #ddd;
            text-decoration: none;
            color: var(--black);
            border-radius: 4px;
        }
        .pagination a.active {
            background-color: var(--main-color);
            color: white;
        }
    </style>
</head>
<body>
    <?php include '../components/admin_header.php'; ?>

    <section class="activity-logs">
        <h1 class="heading">My Activity Log</h1>
        
        <div class="search-container">
            <select id="typeFilter">
                <option value="all">All Types</option>
                <option value="Post">Posts</option>
                <option value="Article">Articles</option>
                <option value="Comment">Comments</option>
                <option value="Like">Likes</option>
            </select>
            <select id="dateFilter">
                <option value="all">All Dates</option>
                <option value="today">Today</option>
                <option value="this_week">This Week</option>
                <option value="this_month">This Month</option>
                <option value="past_3_months">Past 3 Months</option>
            </select>
        </div>

        <table class="activity-logs-table">
            <thead>
                <tr>
                    <th>Type</th>
                    <th>Title</th>
                    <th>Timestamp</th>
                    <th>Action</th>
                    <th>Status</th>
                </tr>
            </thead>
            <tbody>
                <?php if (!empty($activity_logs)): ?>
                    <?php foreach ($activity_logs as $log): ?>
                        <tr>
                            <td><?= htmlspecialchars($log['type']) ?></td>
                            <td><?= htmlspecialchars($log['title']) ?></td>
                            <td><?= date('F j, Y, g:i a', strtotime($log['timestamp'])) ?></td>
                            <td><?= htmlspecialchars($log['action']) ?></td>
                            <td><?= htmlspecialchars($log['status']) ?></td>
                        </tr>
                    <?php endforeach; ?>
                <?php else: ?>
                    <tr><td colspan="5">No activity logs found.</td></tr>
                <?php endif; ?>
            </tbody>
        </table>

        <?php if ($total_pages > 0): ?>
        <div class="pagination">
            <?php for ($i = 1; $i <= $total_pages; $i++): ?>
                <a href="?page=<?= $i ?>&type=<?= urlencode($type_filter) ?>&date=<?= urlencode($date_filter) ?>" 
                   class="<?= ($i === $page) ? 'active' : '' ?>"><?= $i ?></a>
            <?php endfor; ?>
        </div>
        <?php endif; ?>
    </section>

    <script src="../js/admin_script.js"></script>
    <script>
    document.getElementById('typeFilter').addEventListener('change', function () {
        const type = this.value;
        const date = document.getElementById('dateFilter').value;
        window.location.href = `?page=1&type=${encodeURIComponent(type)}&date=${encodeURIComponent(date)}`;
    });

    document.getElementById('dateFilter').addEventListener('change', function () {
        const date = this.value;
        const type = document.getElementById('typeFilter').value;
        window.location.href = `?page=1&type=${encodeURIComponent(type)}&date=${encodeURIComponent(date)}`;
    });
    
    // Preserve selected values on reload
    document.getElementById('typeFilter').value = "<?= $type_filter ?>";
    document.getElementById('dateFilter').value = "<?= $date_filter ?>";
    </script>

</body>
</html>